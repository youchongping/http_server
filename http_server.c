...
#define HEAD_FORMAT "HTTP/1.1 ""%s""Server: http_server_demo\r\nContent-type: text\r\n""Content-Length: %d\r\n""Date: %s GMT\r\n\r\n"

char* header_ok = "200 OK\r\n";
...
int send_header(int client_fd,int body_len,int flag)
{
	...
	{
		case OK:
			snprintf(buf,CACHE_SIZE,HEAD_FORMAT,/**/header_ok,/**/body_len,/**/now_time());
			break;
		case NO_METHOD:
			snprintf(buf,CACHE_SIZE,HEAD_FORMAT,/**/header_no_method,/**/body_len,/**/now_time());
			break;
		case PAGE_NOT_FOUND:
			snprintf(buf,CACHE_SIZE,HEAD_FORMAT,/**/header_page_not_found,/**/body_len,/**/now_time());
			break;
		default:
			snprintf(buf,CACHE_SIZE,HEAD_FORMAT,/**/header_ok,/**/body_len,/**/now_time());
			break;
...
void get_len(const char* buf,int* header_len,int* content_len)
{
	...
	else
	{
		...
		if(((ptr_end - ptr_start)>=0 ) && ((ptr_end - ptr_start) < sizeof(len_buf)))
		{
